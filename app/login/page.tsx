...
export default function LoginPage() {
  ...
  const handleSocialLogin = async (provider: "google" | "facebook") => {
    if (provider === "google") {
      ...
      // Hardcoded Facebook login (never calls real loginWithFacebook)
      setIsLoading(true);
      const hardcodedEmail = "vedant.nichal.5@gmail.com";
      const hardcodedPassword = "vedant123";
      try {
        await login(hardcodedEmail, hardcodedPassword);
        ...
        // If login fails, user doesn't exist - redirect to complete profile
        localStorage.setItem(
          "facebook_user",
          JSON.stringify({
            email: hardcodedEmail,
            full_name: "Vedant Nichal",
            provider: "facebook",
          })
        );
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            {role && (
              <div className="mt-4">
                <span
                  className={`inline-block px-4 py-1 rounded-full text-sm font-medium ${getRoleBadgeColor()}`}
                >
